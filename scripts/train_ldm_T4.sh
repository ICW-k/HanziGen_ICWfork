...
# =============== 基础参数（T4 档：16GB 显存 / 8 核 CPU） ===============
# 适配实例：NVIDIA T4 (16GB / 8核 / 32GB 内存)
# 依据：LDM 在 64x64 潜在空间训练，显存占用极小；batch=64 对 T4 320GB/s 带宽已接近上限
...
BATCH_SIZE=64                          # T4 甜点值（64x64 latent 显存无压力，带宽决定上限）
...
EVAL_BATCH_SIZE=8                      # 评估批大小（T4 16G 保守值）
